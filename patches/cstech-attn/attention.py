...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec | None:
        ...
        if self.sliding_window is not None:
            ...
            # PATCH (2026-08-28, local): hybrid models pad every KV group up to
            # the Mamba page; without a skip-quant list shared_page is None here,
            # the SW block falls to 16 and `unify` PADS the page (non-integer
            # ratio) -> a DFlash drafter needs ~257 blocks/request. Use the
            # Mamba-padded page as the budget instead.
            if shared_page is None:
                shared_page = getattr(vllm_config.cache_config, "mamba_page_size_padded", None)
            ...
            # PATCH: backends advertising only MultipleOf(16) make the helper
            # return 16; expand to the largest multiple of 16 that fits the page.
            if shared_page and sw_per_token > 0:
                _cand = (shared_page // sw_per_token) // 16 * 16
                if _cand >= 16:
                    sw_block_size = max(sw_block_size, _cand)
