...
class SchedulerOffloadConfig(NamedTuple):
    ...
    def from_spec(
        ...
        group_block_sizes = {config.tokens_per_block for config in active_group_configs}
        ...
        supports_partial_tail = (
            # PATCH: the partial-tail hand-off indexes every group's block
            # table by one shared boundary; an inert group has no offloaded
            # blocks to hand off, so disable the optimization outright.
            not inert_groups
            ...
            and not any(config.is_eagle_group for config in active_group_configs)
